...
type CardProps = {
  ...
  heading: string;
  description: string;
  button: CustomButtonProps; // Use the extended type
};
...
export const Layout518 = (props: Layout518Props) => {
  ...
  const cardPositions = cards.map((_, index) =>
    useTransform(scrollYProgress, [0, 0 + index * 0.01, 0.2 + index * 0.5, 1], ["100%", "100%", "0%", "0%"])
  );
            ...
            return (
              <div key={index} className='flex border border-border-primary p-8 text-center'>
                <div className='mx-auto flex max-w-md flex-col items-center justify-center lg:max-w-full'>
                  ...
                  <h3 className='mb-3 text-4xl font-bold leading-[1.2] md:mb-4 md:text-5xl'>{card.heading}</h3>
    ...
    <section ref={containerRef} className='relative py-16 md:py-24 lg:h-[250vh] lg:py-0'>
      <div className='mx-auto w-[90vw] lg:sticky lg:top-0 lg:flex lg:h-screen lg:w-full lg:items-center lg:justify-center lg:overflow-hidden'>
        <div className='grid grid-cols-1 gap-y-6 md:gap-y-8 lg:h-[90vh] lg:w-[90vw] lg:grid-cols-[55vw_35vw] lg:gap-y-0'>
          ...
          <motion.div style={motionCardsStyles} className='grid grid-cols-1 gap-y-6 md:gap-y-8 lg:pl-8'>
            {cards.map((card, index) => {
