          ...
          <div>
            <DialogTrigger asChild>
              <button className='block size-full' onClick={() => setSelectedSlide(0)}>
                <img src={images[0].src} alt={images[0].alt} className='aspect-[5/4] size-full object-cover' />
              </button>
            </DialogTrigger>
          </div>
            ...
            {images.slice(1, 5).map((image, index) => (
              <DialogTrigger key={index} asChild>
                <button className='block w-full' onClick={() => setSelectedSlide(index + 1)}>
                  <img src={image.src} alt={image.alt} className='aspect-[5/4] size-full object-cover' />
                </button>
              </DialogTrigger>
            ))}
          </div>
...
const GallerySheet = ({ images, showAllButton, selectedSlide, setSelectedSlide }: GallerySheetProps) => {
  return (
    <Sheet>
      <SheetTrigger asChild>
        <Button {...showAllButton} className='absolute bottom-4 right-4 z-10 border-border-alternative'>
          {showAllButton.title}
        </Button>
      </SheetTrigger>
