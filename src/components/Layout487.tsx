...
export type Layout487Props = React.ComponentPropsWithoutRef<"section"> &
  Partial<Props>;
...
> = ({ image, tagline, heading, description, buttons, onVisibilityChange }) => {
  ...
  useEffect(() => {
    ...
    return () => unsubscribe();
  }, [opacity, onVisibilityChange]); // Only include specific dependencies
  ...
  return (
    <motion.div
      ...
      style={
        !isMobile
          ? ({ opacity } as { opacity: MotionValue<number> })
          : undefined
      }
