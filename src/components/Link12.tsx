...
export type Links12Props = React.ComponentPropsWithoutRef<"section"> &
  Partial<Props>;
...
const Category = ({ ...category }: CategoryProps) => {
  ...
  const cardLinks = category.links.filter(
    (link) => link.type === CategoryLinkType.Card
  );
  const buttonLinks = category.links.filter(
    (link) => link.type === CategoryLinkType.Button
  );
  const regularButtons =
    cardLinks.length > 0 ? buttonLinks.slice(0, -1) : buttonLinks;
  const storeButton =
    cardLinks.length > 0 ? buttonLinks[buttonLinks.length - 1] : null;
  return (
    <div className="flex flex-col gap-4 text-center">
      {category.heading && (
        <h3 className="red-500 text-md font-bold leading-[1.4] md:text-xl">
          {category.heading}
        </h3>
      )}
              ...
              <div className="relative">
                <CarouselContent className="ml-0">
                  {cardLinks.map((slide, index) => (
                    <CarouselItem
                      key={index}
                      className="basis-[46.5%] px-2 md:basis-1/3 md:px-3"
                    >
